...
ipcMain.on('flac-save', async (event, data) => {
  ...
  function _replyEvent (name, d) {
    if (event && win) {
      try {
        event.reply(name, Object.assign({}, d, {
          url: data.url
        }))
      } catch (err) {
  ...
  if (response) {
    ...
    try {
      win.webContents.session.on('will-download', (downloadEvent, item, webContents) => {
        item.setSavePath(response)

        item.on('updated', _throttle((evt, state) => {
          if (state === 'interrupted') {
            // console.log('下载已经中断，可以恢复')
            // 下载已经中断，可以恢复
            _replyEvent('flac-save-interrupted', {
              path: response
            })
          } else if (state === 'progressing') {
            if (item.isPaused()) {
              // console.log('Download is paused')
              _replyEvent('flac-save-paused', {
                path: response
              })
            } else {
              // console.log('Received bytes: ', item.getReceivedBytes(), item.getTotalBytes())
              // 下载正在进行中
              _replyEvent('flac-save-progress', {
                received: item.getReceivedBytes(),
                total: item.getTotalBytes(),
                path: response
              })
              // console.log('progress: ', parseFloat((item.getReceivedBytes() / item.getTotalBytes()).toFixed(2)))
              // win && win.setProgressBar(parseFloat((item.getReceivedBytes() / item.getTotalBytes()).toFixed(2)))
            }
          } else { }
        }, 1000))
        ...
        item.once('done', (evt, state) => {
          if (state === 'completed') {
            _replyEvent('flac-save-success', {
              path: response
            })
          } else {
            _replyEvent('flac-save-fail', {
              state: state,
              path: response
            })
          }
        })
        _replyEvent('flac-save-start', {
          path: response
        })
      })
    } catch (err) { }
