      ...
      <div class="media_fm_content">
        <keep-alive>
          <component :is="currentCategoryIndex > -1 ? allCategory[currentCategoryIndex].name : additionalPanel"
                     ...
                     @remove-favorite="removeFavorite"></component>
        </keep-alive>
      </div>
    </div>
...
export default {
  ...
  mounted () {
    ...
    global.eventHub.$on('open-anchor-detail-panel', this.openAnchorDetailPanel)
  },
  methods: {
    hideSideBar () {
    ...
    openAnchorDetailPanel (data) {
      this.currentCategoryIndex = 1
      this.showSideBar()
      if (data.page === 'detail') {
        setTimeout(() => {
          global.eventHub.$emit('go-to-anchor-detail', data.data)
        }, 200)
      }
    },
    ...
    setFavorite (res) {
      ipcRenderer.sendSync('fm-set-favorite', {
        data: res.data || this.activeSource,
        type: res.type
      })
      let l = JSON.parse(JSON.stringify(this.favorite[res.type]))
      if (res.data) {
        l.push(res.data)
      } else {
        l.push(this.activeSource)
      }
      this.$set(this.favorite, res.type, l)
      this.$Message.success({
        content: '关注成功'
      })
      // this.favorite[res.type].push(this.activeSource)
    },
    toggleFavorite (type) {
    ...
    removeFavorite (res) {
      ...
      let i = 0
      let outIndex = -1
      for (i; i < this.favorite[res.type].length; i++) {
        if ((this.favorite[res.type][i].url === this.activeSource.url) || (res.data && (this.favorite[res.type][i].id === res.data.id))) {
          outIndex = i
          i = this.favorite[res.type].length
        }
      }
      if (outIndex > -1) {
        let l = JSON.parse(JSON.stringify(this.favorite[res.type]))
  ...
  watch: {
    ...
    'currentCategoryIndex' (val) {
      if (val !== 1) {
        global.eventHub.$emit('reset-anchor-detail-panel')
      }
    }
  }
}
