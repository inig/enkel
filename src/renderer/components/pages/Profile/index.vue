...
<script>
  ...
  export default {
    ...
    async mounted () {
      ...
      ipcRenderer.on('send-group-custom-msg', this.imSendGroupCustomMsg)
    ...
    methods: {
      ...
      findGroupIndex (gid) {
        let outIndex = -1
        let i = 0
        let groupConversations = this.groupConversations
        for (i; i < groupConversations.length; i++) {
          if (groupConversations[i].from_gid == gid) {
            outIndex = i
            i = groupConversations.length
          }
        }
        return outIndex
      },
      async imSendGroupCustomMsg (event, data) {
        // if (this.activeMenuIndex === 3) {
        //   return
        // }
        await this.$store.dispatch('moduleIM/sendGroupCustom', {
          target_gid: data.target_gid,
          target_gname: data.target_gname,
          custom: data
        }).then(async (res) => {
          if (res.data.code == 0) {
      ...
      async imOnMsgReceive (event, data) {
        ...
        if (message.msg_type == 3) {
          ...
          if (targetGroupItemIndex > -1) {
            ...
            this.groupConversations.push({
              from_gid: message.from_gid,
              appkey: message.from_appkey,
              msg_type: 4,
              msgs: [
                message
      ...
      async inviteFriendsHandler (data) {
        await this.imGetGroups()
        // 邀请好友
        let targetGroupItemIndex = -1
        this.groupConversations.some((item, index) => {
      ...
      async imGetSelfInfo (username) {
        return new Promise(async (resolve, reject) => {
          if (this.hasLogin) {
            let _username = username
            if (!_username) {
              _username = this.loginInfo.phonenum
            }
