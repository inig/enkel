...
<script>
  ...
  import { Icon, Input, Button, Drawer, Avatar, Badge, Modal, Checkbox, Alert } from 'view-design'
  ...
  export default {
    ...
    components: {
      Icon, Input, Button, Drawer, Avatar, Badge, Modal, Checkbox, Alert,
      GroupMsgBox,
    ...
    methods: {
      ...
      showAlert (data) {
        this.alertModal.shown = true
        this.alertModal.data = data
        let fromUserInfo = this.membersInfo[data.username]
        if (fromUserInfo) {
          this.$toast.show({
            target: '.alert_tips',
            theme: 'dark',
            iconUrl: '../../../../../static/images/steganography.svg',
            title: (fromUserInfo.nickName || fromUserInfo.username) + ': ',
